...
pub const HTTP_HEADER_USER_AGENT: &str = "forrest.lin";
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			let result = Self::get_polkadot_value();
...
impl<T: Trait> Module<T> {
	...
	fn get_polkadot_value() -> Result<(), Error<T>> {
		debug::info!("Entering get_polkadot_value");
		Self::fetch_github_info();
		Ok(())
	}
	...
	fn fetch_github_info() -> Result<(), Error<T>> {
		...
		if let Some(Some(gh_info)) = s_info.get::<GithubInfo>() {
			...
			let tmp_num = str::from_utf8(&gh_info.data.priceUsd).unwrap_or("0").parse::<f32>().unwrap_or(0.0) as u32;
			debug::info!("I am here:{}", tmp_num);

			Call::<T>::submit_number_unsigned(tmp_num);
			return Ok(());
		}
